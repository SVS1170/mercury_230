...
class mercury230:
    ...
    def connection_test(self=0):
        chunk = addr
        chunk += b'\x00'
        chunk = self.crc16(chunk)
        ser = self.open_port(self=port)
        ser.write(chunk)
        time.sleep(100 / 1000)

    def search_counter(self=0):
        chunk = b'\x00'
        chunk += b'\x08'
        chunk += b'\x05'
        chunk = crc16(chunk)
...
# mercury230.connect(address)
# print(mercury230.get_voltage_C())
mercury230.test_hex_to_bin()
# mercury230.search_counter()
# mercury230.get_P()
